...
export class SubscriptionService {
  ...
  async findAllForUser(userId: number): Promise<Subscription[]> {
    return this.subscriptionRepository.find({ where: { user: { user_id: userId } }, relations: ['user', 'subscribedToUser'] });
  }

  async findOneForUser(subscriptionId: number, userId: number): Promise<Subscription> {
    const subscription = await this.subscriptionRepository.findOne({ where: { subscription_id: subscriptionId, user: { user_id: userId } }, relations: ['user', 'subscribedToUser'] });
    if (!subscription) {
